...
function createBedrockResponse({ bedrockCommand, response, err }) {
  ...
  if (err) {
    ...
    bedrockResponse =
      bedrockCommand instanceof BedrockConverseCommand
        ? new BedrockConverseResponse({ bedrockCommand, response })
        : new BedrockResponse({ bedrockCommand, response })
  }
...
function getBedrockSpec({ commandName }, shim, _original, _name, args) {
  const { agent } = shim
  const { input } = args[0]
  const isConverseCall = CONVERSE_COMMANDS.has(commandName)
  const bedrockCommand = isConverseCall
    ? new BedrockConverseCommand(input)
    : new BedrockCommand(input)
...
function bedrockMiddleware(shim, config, next, context) {
  ...
  if (
    INSTRUMENTED_COMMANDS.has(commandName)
  ) {
